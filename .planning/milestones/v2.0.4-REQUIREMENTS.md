# Requirements Archive: v2.0.4 Foundation

**Archived:** 2026-02-19
**Status:** ✅ SHIPPED

This is the archived requirements specification for v2.0.4.
...
- [x] **VCHK-01**: System checks GitHub releases API for latest MC CLI version — **VALIDATED v2.0.4**
- [x] **VCHK-03**: Version checks use hourly throttling (timestamp-based, no check if <1 hour since last) — **VALIDATED v2.0.4**
- [x] **VCHK-04**: Version checks are non-blocking (never delay mc command execution) — **VALIDATED v2.0.4**
- [x] **VCHK-05**: System uses ETag conditional requests to prevent GitHub API rate limiting — **VALIDATED v2.0.4**
- [x] **VCHK-06**: System caches version data with timestamps for offline resilience — **VALIDATED v2.0.4**
- [x] **VCHK-07**: System compares versions using PEP 440-compliant comparison (packaging library) — **VALIDATED v2.0.4**
- [x] **VCHK-08**: System handles network failures gracefully (show warning, continue with current version) — **VALIDATED v2.0.4**

### Configuration Management

- [x] **UCTL-05**: System persists pinned versions in TOML config (pinned_mc_version field) — **VALIDATED v2.0.4**
- [x] **UCTL-06**: System persists last version check timestamp in TOML config — **VALIDATED v2.0.4**
- [x] **UCTL-09**: System uses file locking to prevent concurrent TOML config writes — **DESCOPED v2.0.4** (single-process assumption per CONTEXT.md)
- [x] **UCTL-10**: System performs atomic writes to TOML config (write temp, rename) — **VALIDATED v2.0.4**

### Runtime Mode Detection

- [x] **RTMD-01**: System detects when running in container (agent mode) vs host — **VALIDATED v2.0.4**
- [x] **RTMD-02**: System disables auto-update when running in container (agent mode) — **VALIDATED v2.0.4**
- [x] **RTMD-03**: Container mode shows informational message: "Updates managed via container builds" — **VALIDATED v2.0.4**
...
| UCTL-09 | Phase 26 | ⚠️ Descoped |
| UCTL-10 | Phase 26 | ✅ Complete |
| RTMD-01 | Phase 27 | ✅ Complete |
| RTMD-02 | Phase 27 | ✅ Complete |
| RTMD-03 | Phase 27 | ✅ Complete |
| VCHK-01 | Phase 28 | ✅ Complete |
| VCHK-03 | Phase 28 | ✅ Complete |
| VCHK-04 | Phase 28 | ✅ Complete |
| VCHK-05 | Phase 28 | ✅ Complete |
| VCHK-06 | Phase 28 | ✅ Complete |
| VCHK-07 | Phase 28 | ✅ Complete |
| VCHK-08 | Phase 28 | ✅ Complete |

**Coverage:**
- v2.0.4 requirements: 14 total
- Mapped to phases: 14/14 (100%)
- Shipped: 13/14 (93% - 1 descoped by design)

---

## Milestone Summary

**Shipped:** 13 of 14 v2.0.4 requirements (93%)
**Descoped:** 1 requirement (UCTL-09 - file locking) based on single-process assumption documented in CONTEXT.md
**Adjusted:** None - all requirements shipped as originally specified or explicitly descoped with rationale
**Dropped:** None

**Key Outcomes:**
- Version checking infrastructure complete with daemon threads, ETag caching, and PEP 440 comparison
- Configuration foundation established with atomic writes and backward compatibility
- Runtime mode detection prevents containerized auto-updates
- 67 comprehensive tests validate all scenarios (100% pass rate)
- Foundation ready for v2.0.5 MC Auto-Update milestone

---
*Archived: 2026-02-19 as part of v2.0.4 milestone completion*
