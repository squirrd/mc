...
def promote_issue(content, issue):
    # Scope search to Active Issues section only — prevents matching a same-named
    # entry that already exists in Completed Issues from a prior cycle.
    active_marker = "## Active Issues"
    completed_marker = "## Completed Issues"

    active_start = content.find(active_marker)
    if active_start == -1:
        print(f"WARNING: Active Issues section not found.", file=sys.stderr)
        return content

    completed_start = content.find(completed_marker)
    search_end = completed_start if completed_start != -1 else len(content)
    active_section = content[active_start:search_end]

    pattern = rf"(### {re.escape(issue)}\n.*?---\n)"
    match = re.search(pattern, active_section, re.DOTALL)
    if not match:
    ...
    # Remove from Active Issues (replace only within the active section)
    active_section_updated = active_section.replace(match.group(1), "", 1)
    content = content[:active_start] + active_section_updated + content[search_end:]
    ...
    # Clean up accumulated blank lines and empty Active Issues placeholder
    content = re.sub(r"(## Active Issues\n)(\n{3,})", r"\1\n", content)
    content = re.sub(r"## Active Issues\n\n---\n", "## Active Issues\n\n(none)\n\n---\n", content)
    return content

def close_issue(content, issue):
    # Scope search to Active Issues section only
    active_marker = "## Active Issues"
    completed_marker = "## Completed Issues"
